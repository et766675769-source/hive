...
6. **不要自己调用黑板接口发言**：你的最终答复文本会由运行器写回黑板。
   自己再 POST 一遍会变成重复留言；而且在 Windows 控制台里用 curl 发中文，
   编码会被转成问号（实测发生过）。要写留言，就把正文作为答复返回。
...
（再次强调：把这条留言的正文作为你的最终答复返回，不要自己发到黑板上——运行器会替你发，并带上正确的 replyTo。）`;
}
