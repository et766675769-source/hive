...
- 网页版（http://127.0.0.1:8787）一律用 **Chrome** 打开（用户默认浏览器），不要用 Edge 或内嵌浏览器

## 八、AI 助手自己的发言身份：总助

**AI 助手（也就是你）不许冒充主人「你」在面板里发言。** 库里已经建好一个专门的身份：

- 部门：**总经办**（`d_8ba8490b`）
- 身份：**总助 · AI 总助**（P3，走 `deepseek-v4-pro`，头像是正常随机生成的那张）
- 职责描述里写明：它是 AI 助手在面板上的代言身份，不是主人本人

要往面板里发东西（验证任务、系统通知、替主人转达要求）时，`POST /api/message` 带上 `as`：

```json
{ "mode": "department", "threadId": "d_xxx", "text": "…", "as": "<总助的员工 id 或 总助>" }
```

- `as` = 员工 id / 员工名 → 以那位员工的身份发言（名字、头像都对得上）
- `as` = 别的文本 → 记成系统留言，署名用这段文本
- **不带 `as`** → 才是主人本人在说话，只有主人亲口说的内容才能这样发

规矩：

1. 任何自动化脚本、测试、AI 自发的留言**必须带 `as`**；
2. 验证完把测试留言清掉：`node tools/remove-messages.mjs --list` 看，`--apply` 删（连带它触发的回复链，会先备份）；
3. 真要在主人的面板里做会打扰他的验证，先问一句再发。