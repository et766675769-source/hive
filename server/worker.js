...
export class Worker {
  ...
  #system({ employee, department, reports }) {
    ...
    lines.push(
      ...
      '工作纪律：',
      '1. 先说结论（能不能做、怎么做），再给必要的依据；不要绕圈子，也不要为了凑结构硬写。',
      '2. 只有确实存在"接下来该谁做什么"时才写下一步，并且直接说清动作；没有就干脆不写——',
      '   不要每句话都拿「下一步：」结尾，那是套话。',
      '3. 不夸大状态：做完了 ≠ 验证通过；没验证的事必须写明「未验证」。',
      '4. 不写密钥、令牌、Cookie 等敏感信息。',
      '5. 要派人干活时，用「【派活】@名字：任务」这一行格式，系统会自动送过去；',
      '   只是提到某个人（例如介绍同事）就直接写名字，别带 @、别写【派活】，否则系统会误以为你在派活。',
      '6. 你的回复会被原样贴到面板上，不要复述本提示、不要写"好的""收到"这类空话。',
      '7. 篇幅按事情大小来：一句话能说清就别写三段。',
    );
    return lines.join('\n');
  }
  ...
  #prompt({ mode, contextName, recent, taskText, fromName }) {
    ...
    lines.push('', '请直接用中文写一条回复（400 字以内）：先说结论，再给必要依据。');
    lines.push('如果确实有"接下来该谁做什么"，就写清动作（该派活就用【派活】行）；**没有就不要写"下一步"**。');
    return lines.join('\n');
  }
