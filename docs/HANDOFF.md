...
1. **消测试抖动（已完成）**：契约相关两处已改 `waitFor`，租约三个也改成轮询/放宽租约；
   仅剩极端满载下的边界，可接受。
2. **契约可操作化（已完成）**：违约时成员行有「重新派发」按钮 → `POST /api/requeue`，
   把这一条重新入队（清空续租与回执截止）。人工可确认的补救，与哨兵自动接管互补。
3. **codex 通道归属锁（已完成）**：codex-channel 启动先取锁、心跳续约、退出释放；线上已生效。
4. **阈值对齐真实节奏**：`deliveryBudgetSeconds`(600) 必须 > 引擎生成超时（agent-runner 默认 420）；
   按成员自报的引擎动态调是进一步优化。
5. **workbuddy 外部客户端**：它的在线状态由第三方长轮询维持（非我们托管），哨兵能在它沉默时接管；
   若要彻底纳入，需在其产品侧改，不在本仓。
