...
1. **延迟主要不在通道**：本机实测 `wss://chatgpt.com/backend-api/codex/responses` 经常连不上，Codex 会退避重连再回落 HTTPS，
   一轮因此要 60–120 秒。这是上游网络，不是黑板或通道的问题（日志里能看到 `Reconnecting… n/5` 与 `os error 10061`）。
2. 交付保证目前是**至少一次**（重试可能产生重复留言），还没有幂等键去重。
3. 断线期间的黑板消息靠 `--catch-up` 补最近一条，不是全量补偿。
4. `turn/interrupt` 已封装但还没有黑板侧入口（缺一条「打断」指令）。
