...
Message Board 本体是网页，但你可以把它当成一个**独立桌面窗口**用：双击即弹窗，无地址栏、无标签页、独立任务栏图标。
...
| **B. WPF + WebView2 外壳**（Windows 原生 exe） | .NET 8 SDK 构建 + WebView2 运行时（Win10/11 自带 Edge 即有） | 想要一个真正的 exe、可发快捷方式 |
...
3. 找不到项目根目录时读环境变量 `MESSAGE_BOARD_ROOT`，端口可用 `MESSAGE_BOARD_PORT` 覆盖。
